...
impl<'c> Translation<'c> {
    ...
    pub fn compute_align_of_type(
        ...
        // (https://github.com/rust-lang/rust/pull/141803) — a toolchain-
        // version fact, not an edition fact; it doesn't exist on any
        // current compiler regardless of requested output edition, so
        // this used to unconditionally emit code that fails to compile
        // whenever `preferred` was true under Edition2021. Always use the
        // `core::mem::align_of` polyfill instead: normal alignment should
        // always be a valid preferred alignment, even if in a few cases
        // it is smaller, so this is safe rather than a hard error.
        if preferred {
            ...
            if let Expr::Cast(ExprCast { expr: ref arg, .. }) = *unparen(&val) {
                ...
                }) = *unparen(arg)
                {
                    match val.base10_parse() {
                        Ok(n @ 0) | Ok(n @ 1) => return Ok(mk().lit_expr(mk().bool_lit(n == 1))),
                        _ => {}
                    }
                }
            }
